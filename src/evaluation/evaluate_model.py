...
def create_user_movie_mappings(test_df: pd.DataFrame, train_df: pd.DataFrame = None):
    ...
    if train_df is not None:
        all_users = sorted(set(test_df['userId'].unique()) | set(train_df['userId'].unique()))
        all_movies = sorted(set(test_df['movieId'].unique()) | set(train_df['movieId'].unique()))
    else:
...
def prepare_ground_truth(
    ...
    logger.info(f"Avg relevant items per user: {np.mean([len(gt) for gt in ground_truth]):.1f}")
...
def compute_metrics(
    ...
    from src.training.metrics import (
        ...
        hit_rate_at_k, map_at_k, coverage
    )
