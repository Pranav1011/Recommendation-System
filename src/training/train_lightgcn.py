...
class LightGCNTrainer:
    ...
    def evaluate(self, k_values=[10, 20, 50]) -> Dict[str, float]:
        ...
        for k in k_values:
            ...
            eval_users = list(self.test_interactions.keys())[:1000]  # Evaluate on 1000 users

            for user_idx in tqdm(eval_users, desc=f"Eval@{k}", leave=False):
                # Get user embedding
                ...
                precisions.append(compute_precision(gt_items, top_k_items))
                ndcgs.append(compute_ndcg(gt_items, top_k_items, k))
                hits.append(compute_hit_rate(gt_items, top_k_items))
